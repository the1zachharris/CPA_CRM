...
angular.module('opsConsoleLite', ['ngAnimate']).controller('dbConsoleController', ['$scope','$http','authorization',
    function($scope,$http,authorization) {
        $scope.dynHeaders =[];
        authorization.check('DBConsole','Read',true);
        ...
        $scope.seedCollection = function(){
            $http.post('/seed/appvconfigs/dropAppend/FieldValue')
            .success(function(d){
